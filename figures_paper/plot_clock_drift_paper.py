...
def plot_corr(st, tr_ref, lags, swin=None, nwin=None,
    ...
    if swin:
        ax.axvline(x=-swin[0],c="b",linewidth=1.0)
        ax.axvline(x=-swin[1],c="b",linewidth=1.0)

        ax.axvline(x=swin[0],c="b",linewidth=1.0)
        ax.axvline(x=swin[1],c="b",linewidth=1.0)

    if nwin:
        ax.axvline(x=-nwin[0],c="r",linewidth=1.0)
        ax.axvline(x=-nwin[1],c="r",linewidth=1.0)
...
# =======================================================
# figures settings
figname = "./clock_drift.pdf"
figsize = (7,7.5)
fs = 9
dpi = 300

xlims = [UTCDateTime("2021-11-01").date, UTCDateTime("2023-01-01").date]
ylims = [-1.5,1.5]

letters = ['a','b','c','d','e','f','g','h','i','j','k','l','m','n']
tmp = r"\textbf{(z)}"
letters = [tmp.replace("z",x) for x in letters]

# files to read
parfiles = [f"obs{x:02}_clock.yml" for x in range(1,13)]
parfiles.append("north_clock.yml")
parfiles.append("south_clock.yml")

for ii, parfile in enumerate(parfiles):
    ...
    # read obs skew info
    if obs_sta not in ["OBS04","NORTH","SOUTH"]:
    ...
    # reverse correlations if neccesary
    sta1, sta2 = pair.split("_")
    if obs_sta not in sta1:
        for tr in st_all:
            tr.data = tr.data[::-1]
        pair = f"{sta2}_{sta1}"
        if apply_window and apply_window == "causal":
            apply_window = "acausal"
        elif apply_window and apply_window == "acausal":
            apply_window = "causal"
    ...
    for tr_day in st_all:
        ...
        # FOR OBS04
        if pair == "ZF.OBS04_ZF.OBS06":
            if tr_day.stats.starttime.date >= UTCDateTime("2022-10-18"):
                ts = ts - 0.7
    ...
    #-----------------------------
    ii += 1
    ax = plt.subplot(7,2,ii)

    sc = ax.scatter(days, tshifts, c=ccoefs, s=8, cmap="viridis", vmin=0,
                    vmax=1.0, alpha=0.8)
...
#            ax.axvline(x=date.date, c="g", label=f"{date.date} ({date.julday})")
    ...
    if obs_sta not in ["OBS04","NORTH","SOUTH"]:
        ...
        ax.plot(recdays, recdaysno * dpd,
                "k", label="Expected clock drift", linewidth=1.,alpha=0.8)

    # labels
    pair = pair.replace("_","-")
    pair = pair.replace("ZF.","")
    pair = pair.replace("KL.","")

    ax.set_title(
        letters[ii-1]+f" {obs_sta} clock drift from {pair}",
        fontsize=fs,
        loc="left",
    )

    if ii > 12:
        ax.set_xlabel("Date [year-month]",fontsize=fs)

        for lab in ax.get_xticklabels(which="major"):
            lab.set(rotation=30,ha="right")
    else:
        ax.set_xticklabels([])

    if (ii % 2) != 0.0:
        ax.set_ylabel("Time diff. [s]",fontsize=fs)
    else:
        ax.set_yticklabels([])
    ...
    if ii == 1:
        ax.legend(ncols=2,
                  loc="upper left",
                  bbox_to_anchor=(-0.1,1.07,1.15,1),
                  fontsize=fs,
                  framealpha=0.8,
                  edgecolor="k",
                  fancybox=False,
    ...
    if ii == 2:
        ...
        cbar.set_ticks([0,0.2,0.4,0.6,0.8,1],labels=[0,0.2,0.4,0.6,0.8,1],
                       fontsize=fs)
        cbar.ax.set_ylabel("Correlation coefficient",fontsize=fs,rotation=0)
        cbar.ax.yaxis.set_label_coords(-0.5,.0)
