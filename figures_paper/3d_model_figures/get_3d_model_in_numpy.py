...
for mtype in ["average","std"]:
    ...
    # ------------------------------------------------
    # save model
    np.save(f"{mtype}_model.npy", vs)
    np.save("lat_mesh.npy", LATMESH)
    np.save("lon_mesh.npy", LONMESH)
    np.save("dep_mesh.npy", DEPMESH)

    if mtype == "average":
        np.save("perturbations_model.npy",vs_per)
