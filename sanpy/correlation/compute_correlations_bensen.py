# TODO: delete hard-coded bad window duration
...
if P.par['bad_windows']:
    ...
    for cmp in P.par["data_cmpts"]:
        ...
        for sta in stations_list:
            file_ = os.path.join(P.par['bad_windows'],
                                 f"{sta.upper()}.{cmp}_OUTLIERS.dat")
            bad_win[cmp][sta] = read_bad_windows(file_)
...
for day in days_to_correlate:
    ...
    for st_win in st.slide(P.par['corr_dur'],
        ...
        for cmp in P.par["data_cmpts"]:
            ...
            for tr in st_win_cmp:
                if tr.stats.npts != P.par['corr_npts']:
                    ...
                    if day in bad_win[cmp][tr_code].keys():
                        for wstart in bad_win[cmp][tr_code][day]:
                            wend = wstart + 3600.0
        ...
        # apply spectral whitening or just filter the ffts
